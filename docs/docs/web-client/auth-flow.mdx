...
By the end of this section you will understand how to use both of the `/assertion/options` and `/assertion/result` methods (as defined by our [API](relying-party/api-def)) to implement a webpage that allows a user to authenticate with a passkey
...
The video below will provide a demonstrate this flow.
...
Because the "global" username has been set to default, the call to `authenticateUser` will pass an empty `userName` field to the RP, indicating the use of a discoverable credential flow.
...
If the username field contain input from the user, the button will invoke a non-discoverable credential flow.
...
:::tip This flow also support security keys
...
There is a variation in autofill `get` requests, that deviates from the standard modal experience. We need to take four additional actions when using an autofill request.

1. We need to invoke the WebAuthn `get` ceremony as soon as the user enters the webpage
2. Need to append additional information to the object that is passed into the `get` method.
3. Create an abort controller object that can terminate the autofill request (more on this below)
4. Add the autofill webauthn property to the username input field
