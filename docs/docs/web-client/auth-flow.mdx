...
:::tip This flow also supports security keys
...
When using autofill there are properties in the `get` method parameters that deviate from the standard modal experience. We need to take four additional actions when using an autofill request.
