...
This section will cover the initial step of a user self-registering their new account with the application. The user will create a new account by entering their username, and registering their first passkey.

## Demonstration

The videos below will demonstrate how a user self registers in the application.

This first video will demonstrate how a user will create an account using a passkey that was registered on a security key.
...
This second video will demonstrate how a user will create a passkey using their on-device authenticator. In our example we will be leveraging Touch ID.
...
3. During these early phases of passkey adoption, you may want to include a blurb that provides a simple explanation on passkeys. The example wording used in our example can be found below
...
The user registration flow shown above does not differ much from what was provided in the previous [implementation guidance for registering on a web client](/docs/web-client/reg-flow).
...
In this case, we will provide a button for security keys, and another for biometrics. They will both utilize the `registerPasskey` method, but pass different values to indicate their authenticator preference. The highlighted line below indicates the authenticator attachment property, which allows your client to set its authenticator preference. An explanation on these values can be found [here](/docs/relying-party/reg-flow#:~:text=authenticatorAttachment%20defines%20the,use%20ANY%20modality.).

:::tip
Note that the concept of `PasskeyServices` was covered in a [previous section](/docs/web-client/api-client#registration-methods). The `getAttestationOptions` method has been expanded in the code example below to demonstrate that we are attempting to invoke different options for the `authenticatorAttachment` property.
:::
