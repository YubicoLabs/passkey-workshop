...
- Any transaction $1000 or over will require that a user had used a high assurance passkey (like a security key) to authenticate into the session
...
The third video will follow a similar flow to the second. The user will be authenticated using a low assurance passkey, and will attempt to make a high risk transaction. In this case, the user will successfully perform step up authentication, and the transaction will successfully process.
